...
export function Navbar() {
  ...
  return (
    <header className="sticky top-0 z-40 border-b border-border/60 bg-canvas/80 backdrop-blur w-full">
        ...
        <div className="flex shrink-0 items-center gap-1.5 sm:gap-3">
          ...
          <Link
            ...
            className={cn(
              ...
              "h-11 inline-flex items-center justify-center whitespace-nowrap px-3 transition-colors duration-150 sm:h-10 sm:px-4"
            )}
          >
          ...
          {/* Mobile hamburger menu toggle - 44x44px minimum touch target */}
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="flex h-11 w-11 items-center justify-center rounded-sm border border-border bg-surface text-ink-muted hover:text-ink sm:hidden transition-colors outline-none"
      ...
      {isOpen && (
        <div className="border-t border-border bg-canvas px-4 py-3 sm:hidden shadow-lg animate-dropdown-in">
